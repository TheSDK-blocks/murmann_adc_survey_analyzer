...
class murmann_adc_survey_analyzer(rtl,eldo,thesdk):
    @property
    def _classfile(self):
    ...
    def run(self,*arg):
        ...
        if self.model=='py':
          ...
          if self.model=='vhdl':
              ...
              # Example of defining supplies (not used here because the example murmann_adc_survey_analyzer has no supplies)
...
if __name__=="__main__":
    ...
    from  murmann_adc_survey_analyzer.controller import controller as murmann_adc_survey_analyzer_controller
    import pdb
    length=1024
    rs=100e6
    indata=np.random.randint(2,size=length).reshape(-1,1);
    #indata=np.random.randint(2,size=length)
    controller=murmann_adc_survey_analyzer_controller()
    controller.Rs=rs
    #controller.reset()
